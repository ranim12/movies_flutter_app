...
class PersonImageList extends StatefulWidget {
  ...
  const PersonImageList({super.key, required this.person});

  @override
  State<PersonImageList> createState() => _PersonImageListState();
}

class _PersonImageListState extends State<PersonImageList> {
  PersonDetailViewModel personDetailViewModel = PersonDetailViewModel();

  @override
  void initState() {
    personDetailViewModel.fetchPersonImage(widget.person.id!);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text("Images", style: AppStyle.instance.bodyXLarge.copyWith(color: AppColors.redColor)),
        SizedBox(
          height: height * 0.02,
        ),
        ChangeNotifierProvider<PersonDetailViewModel>(
            ...
            child: Consumer<PersonDetailViewModel>(builder: (context, value, _) {
              switch (value.personImageList.status) {
                case Status.LOADING:
                  return SizedBox(
                    height: height,
                ...
                case Status.ERROR:
                  return Text(value.personImageList.toString());
  ...
  Widget _upComingMovieList(double height, PersonDetailViewModel value) {
    return Container(
      margin: const EdgeInsets.only(bottom: 20),
      height: height * 0.38,
      width: double.infinity,
      child: PhotoViewGallery.builder(
        itemCount: value.personImageList.data!.profiles!.length,
        builder: (context, index) {
          return PhotoViewGalleryPageOptions.customChild(
            ...
            child: AvaiblePosterPath(
              height: 700,
              width: 500,
              imageUrl: value.personImageList.data!.profiles![index].filePath!,
            ),
          );
        },
